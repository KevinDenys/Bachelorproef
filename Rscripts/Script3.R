...
fsHerinneringen$percentage = (fsHerinneringen$freq / sum(fsHerinneringen$freq))*100
fsVoeding$percentage = (fsVoeding$freq / sum(fsVoeding$freq))*100
fsFotoNotitie$percentage = (fsFotoNotitie$freq / sum(fsFotoNotitie$freq))*100
fsNotitie$percentage = (fsNotitie$freq / sum(fsNotitie$freq))*100
fsInsulineDosis$percentage = (fsInsulineDosis$freq / sum(fsInsulineDosis$freq))*100
fsTipsBijWaarde$percentage = (fsTipsBijWaarde$freq / sum(fsTipsBijWaarde$freq))*100
fsGevDelen$percentage = (fsGevDelen$freq / sum(fsGevDelen$freq))*100
fsMoetZonderInternet$percentage = (fsMoetZonderInternet$freq / sum(fsMoetZonderInternet$freq))*100
fsSmartWatch$percentage = (fsSmartWatch$freq / sum(fsSmartWatch$freq))*100
fsGeschiedenis$percentage = (fsGeschiedenis$freq / sum(fsGeschiedenis$freq))*100
fsDigitalAssistant$percentage = (fsDigitalAssistant$freq / sum(fsDigitalAssistant$freq))*100
fsGrafiek$percentage = (fsGrafiek$freq / sum(fsGrafiek$freq))*100
fsImporteren$percentage = (fsImporteren$freq / sum(fsImporteren$freq))*100
fsExporteren$percentage = (fsExporteren$freq / sum(fsExporteren$freq))*100
fsGlucose$percentage = (fsGlucose$freq / sum(fsGlucose$freq))*100
fsInsulineType$percentage = (fsInsulineType$freq / sum(fsInsulineType$freq))*100
fsInsulinePlaats$percentage = (fsInsulinePlaats$freq / sum(fsInsulinePlaats$freq))*100
fsActiviteiten$percentage = (fsActiviteiten$freq / sum(fsActiviteiten$freq))*100
fsFSL$percentage = (fsFSL$freq / sum(fsFSL$freq))*100
# Score - Op100
...
type <- dataSet$Welk.type.diabetes.heeft.u.